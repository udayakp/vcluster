...
func LoadRuntimeConfig(vClusterName string) (*VirtualClusterConfig, error) {
	...
	if standaloneEnv, ok := os.LookupEnv(constants.VClusterStandaloneEnvVar); ok {
		if standaloneEnv == "true" {
			...
			if err != nil {
				return nil, fmt.Errorf("parse standalone vCluster config: %w", err)
			}
			return vConfig, nil
		}
		...
		if err != nil {
			return nil, fmt.Errorf("parse vCluster config: %w", err)
		}
		return vConfig, nil
	}

	_, found, err := standaloneutil.DetectStandaloneHost()
	if err != nil {
	...
	if found {
		...
		if err != nil {
			return nil, fmt.Errorf("parse standalone vCluster config from systemd: %w", err)
		}
		return vConfig, nil
	}
